...
void semicircle(float radius,float u,float v)
{
    ...
    for (int i=135; i<=315; i++)
   {
      ...
      glVertex2f(u+cos(degInRad)*radius,v+(sin(degInRad))*radius);//100,100 specifies center of the circle
   }
...
glColor3f(0.8,0.498039 ,0.196078);
	glBegin(GL_POLYGON);//core
		...
		glVertex2f(262.5,180.0+i);
		glVertex2f(237.5,180.0+i);


	glEnd();

	glColor3f(1,0.5,0.5);//bonnet
	glBegin(GL_POLYGON);//front
	glVertex2f(237.5,180.0+i);
	glVertex2f(262.5,180.0+i);
	glVertex2f(250,230.0+i);
	glEnd();
...
void mars(float radius)
{
   ...
   {
      float degInRad = i*DEG2RAD;
      glVertex2f(300+f+cos(degInRad)*radius,500-t+(sin(degInRad))*radius);//100,100 specifies center of the circle
   }
    glEnd();
    t=t+0.1;
    f=f+0.1;
